...
export const registerDnsCommands = (program: Command) => {
  program
  ...
  program
    ...
    .action((siteId, zoneId, options) =>
      handleAction(() =>
        api.createDnsRecord(siteId, zoneId, {
          type: options.type,
          name: options.name,
  ...
  program
    ...
    .option('--content <content>', 'Record content')
    .option('--ttl <ttl>', 'Record TTL')
    .action((siteId, zoneId, recordId, options) =>
      handleAction(() =>
        api.updateDnsRecord(siteId, zoneId, recordId, {
          type: options.type,
          name: options.name,
          content: options.content,
          ttl: options.ttl
...
export const registerWebhookCommands = (program: Command) => {
  program
  ...
  program
    ...
    .action((siteId, webhookId, options) =>
      handleAction(() => api.updateWebhook(siteId, webhookId, { url: options.url, event: options.event }))
    );
